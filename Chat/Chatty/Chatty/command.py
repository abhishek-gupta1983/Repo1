...
# symbols which are imported by "from Chatty.command import *"
...
from Chatty.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('Chatty'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='Chatty.config')
...
def bootstrap():
    ...
    By default this runs the function 'Chatty.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-Chatty = Chatty.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="Chatty %s" % version)
    ...
    _read_config(args)
    from Chatty.model import bootstrap_model
...
def start():
    ...
    from Chatty.controllers import Root
