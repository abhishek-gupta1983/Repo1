...
# symbols which are imported by "from TG_App.command import *"
...
from TG_App.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse('TG_App'),
                'config/default.cfg')
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename='TG_App.config')
...
def bootstrap():
    ...
    By default this runs the function 'TG_App.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-TG_App = TG_App.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="TG-App %s" % version)
    ...
    _read_config(args)
    from TG_App.model import bootstrap_model
...
def start():
    ...
    from TG_App.controllers import Root
